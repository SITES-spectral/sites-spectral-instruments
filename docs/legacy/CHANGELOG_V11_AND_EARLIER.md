# Changelog - Legacy Versions (v11 and Earlier)

> **Note**: This file contains the changelog for SITES Spectral versions 11.x and earlier.
> For current changes (v12.x and v13.x), see the main [[../CHANGELOG|CHANGELOG.md]].
...
- **Archived**: 2025-12-27
- **Last Legacy Version**: v11.0.0-alpha.42
- **Current Active Version**: v13.0.0+
- **Migration**: v12.0.0 introduced Normalized Mount Type Codes (BREAKING CHANGE)

---

## Version History

| Version Range | Release Period | Key Features |
|---------------|----------------|--------------|
| v11.0.0-alpha.1 - v11.0.0-alpha.42 | Dec 2025 | Hexagonal Architecture, Domain Authorization, Analytics |
| v10.0.0-alpha.1 - v10.0.0-alpha.17 | Nov-Dec 2025 | ROI Drawing, Admin Panel, Map Integration |
| v8.x - v9.x | Sep-Nov 2025 | UAV Platform, Calibration Workflow, Security |
| v1.x - v7.x | Early 2025 | Initial development, basic CRUD |
...
#### Bug Fix
- **UAV Platform Creation**: Fixed "Create Platform" button being disabled for UAV platforms
  - Issue: `mount_type_code` was marked as required but not shown in the form
  - Fix: Made `mount_type_code` not required (auto-generated by backend)
  - UAV platforms only need Vendor and Model to be valid

- **Satellite Platform Creation**: Same fix applied
  - `mount_type_code` is auto-set to "SAT" by backend
  - Satellite platforms only need Agency, Satellite, and Sensor to be valid

#### Technical Details
- Updated `useTypeRegistry.js`: Changed `mount_type_code` to `required: false` for both UAV and Satellite
- Backend auto-generates position IDs based on platform type (UAV##, SAT)
